  ...
  const handleStartChat = async () => {
    ...
    try {
      ...
      if (currentUserId !== solicitanteId) {
        ...
        const otherUserIds: number[] = [...new Set(msgsResponse.data.map(msg => Number(msg.remetente_id)).filter(uid => uid !== currentUserId))];
      ...
      {showRatingForm && (
        <form onSubmit={handleSendRating} className="rating-form">
          ...
          <div className="star-rating">
            {[1, 2, 3, 4, 5].map(starValue => ( <button type="button" key={starValue} className={starValue <= rating ? 'on' : 'off'} onClick={() => setRating(starValue)}>★</button>))}
          </div>
